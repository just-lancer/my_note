...
-   **使用`Hive`创建一张数据表，并插入数据，最后进行数据查询**
    ...
    **==在执行上述`Hive`命令时，从`xShell`控制页面上可以看到，`Hive`会不停打印`INFO`级别的日志。其原因在于安装部署`Flink`时，配置的`Hadoop`环境变量：`export HADOOP_CLASSPATH=`hadoop classpath`导致的，为了在开发过程中，为了便于查看有效日志，可以对日志信息进行相应的配置。==**
...
由于`metastore`和`hiveserver2`都是基于`Hive`的服务，所以，为了能在相应的节点启动对应的服务，首先需要将`Hive`进行分发。
...
-   **由于在`metastore`独立服务模式下，所有的客户端都需要访问这唯一的`metastore`服务，进而获取元数据库的数据，所以，在开启`Hive`服务的节点上，需要配置访问`metastore`服务的地址和端口号**
    ...
    -   **因此，需要在`hadoop132`、`hadoop133`和`hadoop134`节点的`Hive`配置文件`hive-site.xml`中添加以下内容：**
...
**==需要额外说明的是，`Hive`元数据信息存储在`MySQL`中，但`Hive`要求数据库级别的字符集必须是`latin1`，并且默认情况下，字段的字符集也是`latin1`。但是由于`latin1`斌不支持中文，所以，使用在使用`JDBC`客户端连接`Hive`时，中文注释会出现乱码。==**

**解决方案的关键在于修改`Hive`元数据库中相应表的数据，修改`Hive`的字符集**

-   **修改`hive-site.xml`文件中元数据库连接配置：**

    ```xml
    <!-- jdbc连接的URL -->
    <property>
        <name>javax.jdo.option.ConnectionURL</name>
        <value>jdbc:mysql://hadoop132:3306/metastore?useSSL=false&amp;allowPublicKeyRetrieval=true&amp;useUnicode=true&amp;characterEncoding=UTF-8</value>
    </property>
    ```

    **其中，参数`allowPublicKeyRetrieval=true`的配置，是为了避免出现数据库连接失败报错：`Public Key Retrieval is not allowed`。出现这个问题原因是：`MySQL 8.x`以上版本默认使用`sha256_password`认证，密码在传输过程中必须加密保护，如果无法使用`TLS`，就需要使用`RSA`公钥加密。**

    **参数`useUnicode=true`和`characterEncoding=UTF-8`使得数据库连接使用`Unicode`的`UTF-8`字符集。**

-   **`Hive`的元数据库`metastore`，修改相应表的数据值**
...
-   **修改完成，重启`Hadoop`和`Hive`，新建的数据库、数据表的中文注释就可以正常识别，但已存在的数据库、数据表的中文注释还是无法被识别**
